...
## Overview
Marketing site for a local Sanitär & Badumbau company in Basel, Switzerland. Primary goal: lead generation via Google Ads. The `/badumbau` page is the main landing page for ad traffic.
...
src/
  ...
  pages/
    ...
    badumbau.astro        # Google Ads landing page — primary conversion page
    impressum.astro
    datenschutz.astro
  components/
    ...
    Services.astro        # 3 service cards (featured Badumbau, Sanitär, Heizung)
...
- `home`: shows Startseite, Leistungen, Über uns, Badumbau (blue+bold), Phone
...
## Sitemap
- `/sitemap-index.xml` — auto-generated by `@astrojs/sitemap` (covers `/`, `/badumbau`, `/impressum`, `/datenschutz`)
- `/robots.txt` — Allow all, sitemap pointer
...
7. **Google Ads SEO** — verify `<title>` and `<meta description>` on `/badumbau` contain keyword "Badezimmer renovieren Basel"
8. **Google rating** — update "4.9 / 5 · 47 Google-Rezensionen" in Projects.astro and TrustStrip.astro once real reviews exist
